...
---
또 다른 쿼리 안에 `중첩된 쿼리`
- 중간 단계에서의 Data Transformation에 활용된다.
...
## SELECT 문 내 서브쿼리
---
- 단일 값 (Single value)를 리턴한다: 개별 값들과 비교하기 위해 집계 함수 값을 포함한다.
    - Otherwise, ERROR!
...
## FROM 문 내 서브쿼리
---
- 데이터를 사전 필터링 하거나 `SELECT` 이전에 데이터를 변형
- 하나의 `FROM` 문에 여러 서브쿼리를 만들 수 있다
    - Alias (AS) 별칭 처리하기
    - JOIN 조인
